...
class TelinkBleModule(reactContext: ReactApplicationContext) :
    ...
    override fun onProvisionSuccess(event: ProvisioningEvent) {
        if (currentDevice!!.state == NetworkingState.PROVISIONING) {
            ...
            val appKeyIndex: Int = meshInfo.defaultAppKeyIndex
            val bindingDevice =
                BindingDevice(nodeInfo.meshAddress, nodeInfo.deviceUUID, appKeyIndex)
    ...
    private fun checkPermission(): Boolean {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            val read =
                ContextCompat.checkSelfPermission(reactApplicationContext, READ_EXTERNAL_STORAGE);
            val write =
    ...
    private fun requestPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            try {
                ...
                intent.data =
                    Uri.parse(String.format("package:%s", reactApplicationContext.packageName));
    ...
    fun getMeshNetwork(promise: Promise) {
        ...
        meshNetworkInfo.putString(
            "appKey",
            (Arrays.bytesToHexString(meshNetwork!!.appKeyList[0].key, ""))
        )
        meshNetworkInfo.putString(
            "netKey",
            Arrays.bytesToHexString(meshNetwork.meshNetKeyList[0].key, "")
        )
    ...
    @ReactMethod
    fun otaDeviceByMacAddress(meshAddress: Int, filePath: String, mac: String, promise: Promise) {
        ...
        try {
            ...
            stream.close()
            val connectionFilter =
                ConnectionFilter(ConnectionFilter.TYPE_MAC_ADDRESS, mac)
            val parameters = GattOtaParameters(connectionFilter, mFirmware)
            MeshService.getInstance().startGattOta(parameters)
            promise.resolve("start ota")
        } catch (e: IOException) {

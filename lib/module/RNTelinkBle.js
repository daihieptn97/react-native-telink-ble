...
export class RNTelinkBle extends TelinkBleModule {
  /**
  ...
  onNodeResetSuccess(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_NODE_RESET_SUCCESS, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onNodeResetFailed(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_NODE_RESET_FAILED, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onDeviceFound(callback) {
    const subscription = this.eventEmitter.addListener(Platform.OS === 'android' ? BleEvent.EVENT_DEVICE_FOUND : BleEvent.EVENT_BINDING_SUCCESS, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onScanningTimeout(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_SCANNING_TIMEOUT, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onMeshConnected(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_MESH_NETWORK_CONNECTION, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onSetGroupSuccess(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_SET_GROUP_SUCCESS, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onSetGroupFailed(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_SET_GROUP_FAILED, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onAndroidScanFinish(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_ANDROID_SCAN_FINISH, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onReceiveUnknownNotificationMessage(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_TYPE_NOTIFICATION_MESSAGE_UNKNOWN, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onOtaDeviceSuccess(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_TYPE_OTA_SUCCESS, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onOtaDeviceFail(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_TYPE_OTA_FAIL, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onOtaDeviceProgress(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_TYPE_OTA_PROGRESS, callback);
    return () => {
      subscription.remove();
    };
  }
  ...
  onGetDeviceOnline(callback) {
    const subscription = this.eventEmitter.addListener(BleEvent.EVENT_DEVICE_ONLINE, callback);
    return () => {
      subscription.remove();
    };
  }

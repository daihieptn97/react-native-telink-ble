...
public class NodeInfo implements Serializable {
  ...
  private final OfflineCheckTask offlineCheckTask = new OfflineCheckTask() {
    @Override
    public void run() {
      onOff = -1;
      MeshLogger.log("offline check task running");
      TelinkBleApplication.Companion.getInstance().dispatchEvent(new NodeStatusChangedEvent(TelinkBleApplication.Companion.getInstance(), NodeStatusChangedEvent.EVENT_TYPE_NODE_STATUS_CHANGED, NodeInfo.this));
    }
  };
  ...
  public void setOnOff(int onOff) {
    ...
    if (publishModel != null) {
      Handler handler = TelinkBleApplication.Companion.getInstance().getOfflineCheckHandler();
